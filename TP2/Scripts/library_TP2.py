...
def inliers(m1,m2, Treshold, n, x, y):
    """Cette fonction compare deux modèles et nous indique lequel est le meilleur 
    ...
    Returns:
        list: renvoie le meilleur modèle
    """
    ...
    for _ in range(10):
        ...
        for i in range(n):
            if abs(beta1_m1*x[i] + beta2_m1 - y[i]) < Treshold:
                n_m1 += 1
            
            if abs(beta1_m2*x[i] + beta2_m2 - y[i]) < Treshold:
                n_m2 += 1
